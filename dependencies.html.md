...
``` python
# Test that titles containing Mermaid-special characters are quoted/escaped.
# These previously caused "Parse error ... got 'PS'" failures (e.g. parentheses).
special_char_graph = DependencyGraph()

special_modules = [
    ("manifest", 'Manifest Format (v2.0)'),       # parentheses
    ("config", "Settings [advanced]"),            # square brackets
    ("schema", "Schema {strict}"),                # curly braces
    ("io", "Read & Write"),                       # ampersand
    ("quotes", 'The "quoted" module'),            # embedded double quotes
]

for name, title in special_modules:
    special_char_graph.add_module(ModuleInfo(
        path=Path(f"nbs/{name}.ipynb"), name=name, title=title,
        description=None, functions=[], classes=[], variables=[], imports=[]
    ))

special_char_graph.add_dependency(
    ModuleDependency(source="manifest", target="schema", import_type="from", imported_names=["*"])
)

diagram = generate_mermaid_diagram(special_char_graph, direction="LR")
print(diagram)

# Every node label must be wrapped in quotes so special characters don't break parsing,
# and embedded double quotes must be escaped to Mermaid's entity code.
assert '["manifest<br/>Manifest Format (v2.0)"]' in diagram
assert '["config<br/>Settings [advanced]"]' in diagram
assert '["schema<br/>Schema {strict}"]' in diagram
assert '["io<br/>Read & Write"]' in diagram
assert '["quotes<br/>The #quot;quoted#quot; module"]' in diagram
# A raw double quote inside the label text must not survive
assert 'The "quoted"' not in diagram
```
